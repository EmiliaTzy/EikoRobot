...
async def _(event):
    ...
    if not event.is_reply:
        await event.reply("Balas ke file untuk mengkompres.")
        return
    if event.is_group:
    ...
    mone = await event.reply("⏳️ Mohon Tunggu...")
...
async def _(event):
    ...
    mone = await event.reply("Sedang Diproses...")
    if not os.path.isdir(TEMP_DOWNLOAD_DIRECTORY):
        os.makedirs(TEMP_DOWNLOAD_DIRECTORY)
    if event.reply_to_msg_id:
        start = datetime.now()
        ...
        await event.reply("Membongkar zip sekarang 😌")
...
__help__ = """
Hai, saya dapat mengkonversi file disini..
 ❍ /zip*:* Balas ke file telegram untuk mengkompres dalam format.zip
